...
var searchUser = function(data , callback){
  var statement = "SELECT *,DATE_FORMAT(birthday,'%d/%m/%Y') as birthday1 FROM user where isActive = 1 and fullname like '%"+data+"%' or (type like '%"+data+"%' and type <> 'manager');"
  console.log(statement);
  connection.query(statement, function(error, result){
    if (error) {
      throw error;
...
var complete = function(data,callback){
  connection.query('SELECT fullname,type from user where isActive = 1 and fullname like "%'+data+'%"', function(err, rows) {
    if (err) {
      throw error;
    }
      callback(null,rows);
...
var complete1 = function(data,callback){
  connection.query('SELECT type from user where isActive = 1 and type like "%'+data+'%"', function(err, rows) {
    if (err) {
      throw error;
    }
      callback(null,rows);
